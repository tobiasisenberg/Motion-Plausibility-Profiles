...
## Configuration
To configure the script, adjust the flags ```create*``` at the top of the script as needed (but the script runs with the default configuration out of the box). Also needs CSV data files (names as specified in the ```familyFiles``` list in the script, so rename the ```observations-243682.csv``` (or similar) files from the download to ```family.csv``` for easier association) downloaded from https://www.inaturalist.org/observations/export (if using different files adjust the ```familyFiles``` list).
